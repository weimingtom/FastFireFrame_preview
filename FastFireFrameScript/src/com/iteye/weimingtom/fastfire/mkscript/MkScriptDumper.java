...
import com.iteye.weimingtom.fastfire.mkscript.parser.Label;
import com.iteye.weimingtom.fastfire.mkscript.parser.LabelRef;
import com.iteye.weimingtom.fastfire.mkscript.parser.Lexer;
import com.iteye.weimingtom.fastfire.mkscript.parser.ValueOrNumber;
...
public class MkScriptDumper {
	private static final boolean DEBUG_MENU = false; //-->menu:
	...
	private FFFFile reader;
	
	private boolean isError() { 
		return nerror != 0; 
	}
	
	public MkScriptDumper(FFFResource res) {
		this.res = res;
		reader = null;
		nerror = 0;
		then_index = 0;
	...
	private boolean openValueTable() {
		FFFFile fin = new FFFFile(res, getValueFile());	
		if (!fin.isOk()) {
			fin.close();
			return false;
		}
		...
		for (Label lp : labels) {
			if (lp.label.equals(label)) {
				if (lp.ref == null) {
					...
					while (chain != null) {
						if (DEBUG_REF_VALUE) {
							FFFLog.trace("===>ref label_ref_address:" + chain.label_ref_address);
						}
						LabelRef next = chain.next;
						if (DEBUG_WRITE_LABEL) {
							FFFLog.trace("<<<< write address: " + chain.label_ref_address + " , value: " + command_buffer.position());
						}
					...
					if (DEBUG_REF_VALUE) {
						FFFLog.trace("===>set label_ref:" + label + "\t" +
								lp.line + "\t" +
	...
	private void findLabel(String label, int reference_address) {
		if (DEBUG_WRITE_LABEL) {
			FFFLog.trace("<<<< write address: " + reference_address + " , value: " + 0);
		}
		...
		for (Label lp : labels) {
			if (lp.label.equals(label)) {
				if (lp.ref != null) {
					...
					if (DEBUG_REF_VALUE) {
						FFFLog.trace("===>new LabelRef:" + "\t" + 
								lp.label + "\t" + 
								lp.jmp_addr);
					}
				} else {				
					if (DEBUG_WRITE_LABEL) {
						FFFLog.trace("<<<< write address: " + reference_address + 
		...
		if (DEBUG_REF_VALUE) {
			FFFLog.trace("===>new Label:" + label + "\t" + reader.getLineNo());
		}
	}
	...
	public ByteBuffer duplicateBuffer() {
		...
		FFFLog.trace("header magic == " + MkScriptType.SCRIPT_MAGIC);	
		bytes.putInt(command_buffer.position());
		FFFLog.trace("header ncommand == " + (command_buffer.position()));
		...
		if (p == null || lexer.getString() != null) {
			...
			for (int no = 0; (str = reader.getString()) != null; no++) {
				if (MkScriptDumper.DEBUG_MENU) {
					FFFLog.trace("-->menu:" + str + "length=" + str.length());
				}
