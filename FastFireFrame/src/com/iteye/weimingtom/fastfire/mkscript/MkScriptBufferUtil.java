package com.iteye.weimingtom.fastfire.mkscript;
...
public class MkScriptBufferUtil {
	private static final boolean DEBUG_WRITE_COMMAND = false;
		...
		switch(id) {
			case MkScriptType.SET_VALUE_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<SET_VALUE_CMD : " + setValueCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.CALC_VALUE_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<CALC_VALUE_CMD : " + calcValueCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.MENU_ITEM_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<MENU_ITEM_CMD : " + menuItemCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.UPDATE_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<UPDATE_CMD : " + updateCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.CLEAR_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<CLEAR_CMD : " + clearCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.MUSIC_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<MUSIC_CMD : " + musicCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.SOUND_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<SOUND_CMD : " + soundCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.SLEEP_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<SLEEP_CMD : " + sleepCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.WIPEIN_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<WIPEIN_CMD : " + wipeinCmd);
				}
				if (isWrite) {
			...
			case MkScriptType.WIPEOUT_CMD:
				if (DEBUG_WRITE_COMMAND) {
					FFFLog.trace("<<WIPEOUT_CMD : " + wipeoutCmd);
				}
				if (isWrite) {
